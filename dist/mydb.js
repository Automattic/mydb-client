...
/**
 * Converts an 10gen obj into  regular obj.
 * For example:
 *
 *     { _id: { $oid: "" } } => { _id: "" }
 *
...
Document.prototype.bson = function(obj){
  for (var i in obj) {
    if (obj.hasOwnProperty(i)) {
      if (obj[i] && !Array.isArray(obj) && 'object' == typeof obj[i]) {
        if (obj[i].$oid) {
          obj[i] = obj[i].$oid;
        } else {
          obj[i] = this.bson(obj[i]);
        }
      }
    }
  }
...
Document.prototype.onOp = function (query, mod, implicit) {
  debug('got %soperation %j', implicit ? 'implicit ' : '', mod);
  query = this.bson(query);
  mod = this.bson(mod);
    ...
    for (var key in mod.$rename) {
      if (mod.$rename.hasOwnProperty(key)) {
        (function (k) {
          ...
          self.onOp(query, unsetOp, true);
          self.onOp(query, setOp, true);
